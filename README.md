...
| `SUPABASE_URL` | Supabase Project URL (서버리스 함수용) |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase Service Role Key |
| `GOOGLE_CREDENTIALS_JSON` | credentials.json 파일 전체 내용 (JSON 문자열) |
...
CREATE TABLE profiles (
  ...
  name TEXT, organization TEXT, contact TEXT,
...
BEGIN
  INSERT INTO profiles (id, name, organization, contact)
  VALUES (NEW.id,
...
1. Supabase → Authentication → Users → `Invite user`로 마스터 이메일 계정 생성
